...
export const DocsCloudApiAxiosParamCreator = function (configuration?: Configuration) {
    ...
    return {
         ...
         * @summary Check the DocsCloud server health
         * @param {*} [options] Override http request option.
         * @throws {RequiredError}
         * REST API Reference for checkHealth operation
         * @see https://api.onlyoffice.com/docspace/api-backend/usage-api/check-health/
         */
        checkHealth: async (options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {

            const localVarPath = `/api/2.0/settings/docscloud/healthcheck`;
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
            if (configuration) {
...
export const DocsCloudApiFp = function(configuration?: Configuration) {
    ...
    return {
        ...
        async checkHealth(options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<void>> {
            ...
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['DocsCloudApi.checkHealth']?.[localVarOperationServerIndex]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, localVarOperationServerBasePath || basePath);
        },
        /**
         * Starts generating the DocsCloud user quota report as an xlsx file and saves it in My Documents.
...
export class DocsCloudApi extends BaseAPI {
    ...
    public checkHealth(options?: RawAxiosRequestConfig) {
        return DocsCloudApiFp(this.configuration).checkHealth(options).then((request) => request(this.axios, this.basePath));
    }
