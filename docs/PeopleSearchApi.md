...
|[**getAdvanced**](#getadvanced) | **GET** /api/2.0/people/status/{status}/search | Search users by status filter|
|[**getFullByFilter**](#getfullbyfilter) | **GET** /api/2.0/people/filter | Search users with detaailed information by extended filter|
|[**getPeopleSearch**](#getpeoplesearch) | **GET** /api/2.0/people/search | Search users (using query parameters)|
...
# **getAdvanced**
> EmployeeFullArrayWrapper getAdvanced()
...
const { status, data } = await apiInstance.getAdvanced(
    status,
    query
);
```
...
# **getFullByFilter**
> EmployeeFullArrayWrapper getFullByFilter()
...
const { status, data } = await apiInstance.getFullByFilter(
    employeeStatus,
    groupId,
    activationStatus,
    employeeType,
...
# **getPeopleSearch**
> EmployeeArrayWrapper getPeopleSearch()
...
const { status, data } = await apiInstance.getPeopleSearch(
    query
);
```
